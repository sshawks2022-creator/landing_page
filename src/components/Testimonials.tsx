...
const Testimonials = () => {
  const testimonials = [{
    quote: "SS Security Agency has been instrumental in securing our facilities. Their professional approach and military-grade expertise give us complete peace of mind.",
    author: "Rajesh Kumar",
    position: "Security Head, DRDO",
    rating: 5
  }, {
    quote: "The level of professionalism and dedication shown by the SS Security team is unmatched. They've transformed our security infrastructure completely.",
    author: "Priya Sharma",
    position: "Facility Manager, IOCL",
    rating: 5
  }, {
    quote: "Working with SS Security has been a game-changer. Their veteran-led approach brings discipline and reliability that we can always count on.",
    author: "Amit Patel",
    position: "Operations Director, Karnataka Govt",
    rating: 5
  }, {
    quote: "Outstanding service and commitment to excellence. SS Security Agency sets the standard for what professional security should look like.",
    author: "Sanjay Reddy",
                      ...
                      <Card className="border-2 border-border hover:border-accent transition-all duration-300 red-glow-hover h-full">
                        <CardContent className="p-8 flex flex-col h-full">
                          ...
                          <div className="flex items-center gap-1 mb-4">
                            {[...Array(testimonial.rating)].map((_, i) => <span key={i} className="text-accent text-xl">★</span>)}
                          </div>
                          <div>
              ...
              {[...brandLogos, ...brandLogos].map((brand, index) => (
                ...
                >
                  <div className="flex items-center justify-center h-24 bg-muted rounded-lg border border-border hover:border-accent transition-all duration-300 group">
                    <div className={`text-xl md:text-2xl font-heading font-bold text-primary/${brand.opacity} group-hover:text-primary transition-colors`}>
                      {brand.name}
                    </div>
                  </div>
                </div>
